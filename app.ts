// Hard-coded attack sequence for Pathfinder
interface AttackSequence {
    name: string;
    attackBonus: number;  // Base attack bonus
    damageBase: string;   // e.g., "1d8" for longsword
    damageBonus: number;  // Strength modifier, etc.
}
...
function rollDice(diceNotation: string): { total: number; rolls: number[] } {
...
function getActiveBonuses(): { attack: number; damage: number; description: string[] } {
    const flankingCheckbox = document.getElementById('flanking') as HTMLInputElement;
    const blessCheckbox = document.getElementById('bless') as HTMLInputElement;
